    ...
    >
      <div className="flex justify-between items-center w-full px-4 sm:px-8 py-4 max-w-7xl mx-auto min-h-[72px] sm:min-h-[80px]">
        ...
        >
          ...
          >
            <span className="text-white font-headline font-bold text-base sm:text-xl">
              A
            </span>
          </div>
          <span className="text-sm sm:text-xl lg:text-2xl font-bold tracking-tighter text-primary font-headline whitespace-normal lg:whitespace-nowrap line-clamp-2 lg:line-clamp-none">
            Adhoc Skills Hub
          </span>
        </Link>
        ...
        <div className="hidden lg:flex items-center gap-4">
          ...
          {!isAuthenticated ? (
            <>
              <Link
                to="/auth"
                className="px-5 py-2 text-primary font-semibold hover:opacity-80 transition-all duration-200 active:scale-95"
              >
                Login
              </Link>
              <Link
                to="/auth/register"
                className="px-6 py-2 signature-gradient text-white rounded-lg font-semibold hover:opacity-90 transition-all duration-200 active:scale-95 shadow-md"
              >
                Sign Up
              </Link>
            </>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                >
                  ...
                  <ChevronDown
                    className={`w-4 h-4 text-on-surface-variant transition-transform ${isDropdownOpen ? "rotate-180" : ""}`}
                  />
                </button>
              ...
              {!isAuthenticated ? (
                ...
                <>
                  {user?.role === "admin" && (
                    <Link
                      to="/admin"
                      className="rounded-md px-3 py-2 text-sm font-medium text-primary hover:bg-surface-dim"
                      onClick={() => setIsOpen(false)}
                    >
                      Admin Console
                    </Link>
                  ...
                  <button
                    onClick={handleSignOut}
                    className="mt-1 rounded-md px-3 py-2 text-left text-sm font-semibold text-error hover:bg-error-container"
                  >
                    Sign Out
                  </button>
                </>
