...
function AdminBlogsContent() {
  ...
  const fetchBlogs = async () => {
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL || "http://localhost:5000/api"}/blogs/admin/all`,
        {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = StorageService.getToken();
      const res = await fetch(
        `${import.meta.env.VITE_API_URL || "http://localhost:5000/api"}/blogs/${id}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
      ...
      {isEditing ? (
        <div className="bg-surface-container-low border border-surface-dim/20 rounded-[2rem] p-8 shadow-sm">
          <div className="flex justify-between items-center mb-6">
            ...
            <button
              ...
              className="p-2 bg-surface-dim rounded-full text-on-surface-variant hover:text-error transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <form onSubmit={handleSave} className="space-y-6">
            ...
            <div className="flex justify-end gap-4 pt-4 border-t border-surface-dim/20">
              <button
                type="button"
                onClick={() => setIsEditing(false)}
                className="px-6 py-3 border border-surface-dim rounded-xl font-bold hover:bg-surface-dim"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="flex items-center gap-2 px-8 py-3 signature-gradient text-white rounded-xl font-bold shadow-lg hover:opacity-90"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {blogs.map((blog) => (
            <div
              key={blog.id}
              className="bg-surface-container-low border border-surface-dim/20 rounded-2xl p-6 flex flex-col hover:shadow-lg transition-shadow"
            >
              ...
              <div className="flex items-center gap-2 mb-4">
                <span
                  className={`px-2 py-1 rounded text-xs font-bold ${blog.status === "published" ? "bg-emerald-500/20 text-emerald-600" : "bg-yellow-500/20 text-yellow-600"}`}
                >
              ...
              <div className="mt-auto pt-4 border-t border-surface-dim/20 flex justify-end gap-3">
                <button
                  ...
                  className="p-2 text-secondary hover:bg-surface-dim rounded-lg transition-colors"
                >
