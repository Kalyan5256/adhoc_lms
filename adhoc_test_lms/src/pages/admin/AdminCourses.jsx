...
function AdminCoursesContent() {
  ...
  const handleOpenModal = (course = null) => {
    ...
    if (course) {
      ...
      let priceValue = "";
      const p1 =
        course.price_1month || (course.prices ? course.prices["1month"] : null);
      const p3 =
        course.price_3months ||
        (course.prices ? course.prices["3months"] : null);
      const p6 =
        course.price_6months ||
        (course.prices ? course.prices["6months"] : null);

      if (course.allowed_plan === "3months")
        priceValue = p3 || p1 || course.price;
      else if (course.allowed_plan === "6months")
        priceValue = p6 || p3 || p1 || course.price;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      payload.price_1month =
        formData.allowed_plan === "1month"
          ? priceNum
          : editingCourse?.price_1month || 0;
      payload.price_3months =
        formData.allowed_plan === "3months"
          ? priceNum
          : editingCourse?.price_3months || 0;
      payload.price_6months =
        formData.allowed_plan === "6months"
          ? priceNum
          : editingCourse?.price_6months || 0;
