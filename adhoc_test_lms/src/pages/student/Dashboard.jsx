...
function DashboardContent() {
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: FileText,
      accent: "primary",
      trend: "Based on course progress",
    },
    {
      label: "Certificates",
      ...
      <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-fit lg:mr-20">
              ...
              */}
              <div className="flex-1 lg:flex-none lg:min-w-[220px] bg-surface-container-low px-4 sm:px-6 py-4 rounded-2xl border border-surface-dim/20 flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] font-bold text-secondary uppercase tracking-widest leading-none mb-1 whitespace-nowrap">
                    Global Standing
                  </p>
                  <p className="text-primary font-headline font-bold text-sm sm:text-base">
                    Top 5% of Scholars
                  </p>
                </div>
              </div>
          ...
          {stats.map((stat, i) => {
            ...
            return (
              <motion.div
                ...
                className="bg-surface-container-lowest p-4 sm:p-8 rounded-2xl sm:rounded-[2.5rem] border border-surface-dim/20 shadow-xl shadow-primary/5 transition-all group"
              >
                <div
                  className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl ${stat.accent === "primary" ? "bg-primary-fixed text-primary" : "bg-surface-container-high text-secondary"} flex items-center justify-center mb-4 sm:mb-6 group-hover:scale-110 transition-transform`}
                >
                  <Icon className="w-5 h-5 sm:w-6 sm:h-6" />
                </div>
                <p className="text-[8px] sm:text-[10px] font-bold text-outline uppercase tracking-[0.2em] mb-1">
                  {stat.label}
                </p>
                <p className="text-2xl sm:text-4xl font-headline font-extrabold text-primary tracking-tighter">
                  {stat.value}
                </p>
                <p className="hidden sm:block text-xs text-on-surface-variant mt-2">
                  {stat.trend}
                </p>
              </motion.div>
            );
            ...
            {recentActivity.length === 0 ? (
              ...
              <div className="space-y-4">
                {recentActivity.map((activity) => (
                  <div
                    key={activity.id}
                    className="flex items-center gap-4 p-3 rounded-xl hover:bg-surface-container-high transition-all"
                  >
                    <div
            ...
            {upcomingDeadlines.length === 0 ? (
              ...
              <div className="flex-1 flex flex-col gap-4">
                {upcomingDeadlines.map((deadline) => (
                  <div
                    key={deadline.id}
                    className="py-6 px-5 min-h-[120px] flex flex-col rounded-2xl bg-surface-container-high/40 border border-surface-dim/10 hover:bg-surface-container-high/60 transition-all group"
                  >
        ...
        >
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
            <div>
              <h2 className="text-2xl sm:text-3xl font-headline font-bold text-primary italic">
                Active Protocols
              </h2>
            ...
            <Link
              to="/catalog"
              className="text-[10px] sm:text-xs font-bold text-primary uppercase tracking-[0.2em] border-b-2 border-primary pb-1 hover:opacity-70 transition-all flex items-center gap-1"
            >
          ...
          {!isHydrated ? (
            ...
            >
              ...
              <p className="text-on-surface-variant max-w-sm mx-auto mb-10 font-medium">
                Your learning profile is currently idle. Access the global
                catalog to initialize your first curriculum.
              </p>
              <Link
                to="/catalog"
                className="px-10 py-5 signature-gradient text-white rounded-2xl font-bold text-lg shadow-xl inline-flex items-center gap-2 group"
              >
                Initialize Journey
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {recommendedCourses.map((course) => (
                <div
                  key={course.id}
                  className="bg-surface-container-lowest rounded-2xl p-5 border border-surface-dim/20 hover:border-primary/30 transition-all"
                >
